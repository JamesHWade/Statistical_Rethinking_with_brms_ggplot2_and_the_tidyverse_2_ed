...
>
> <cite>[@mcelreathStatisticalRethinkingBayesian2020] p. 20</cite>
...
<iframe width="600" height="350" src="https://www.youtube.com/embed/R1vcdhPBlXA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen>
...
![](static/02_Lecture-Slides_The-Garden-of-Forking-Data.pdf){width="600" height="400" fig-align="center"}

## The garden of forking data

Gelman and Loken [-@gelmanGardenForkingPaths2013] wrote a [great paper of a similar name](https://stat.columbia.edu/~gelman/research/unpublished/p_hacking.pdf) and topic. The titles from this section and Gelman and Loken's paper have their origins in the short story by [Jorge Luis Borges](https://en.wikipedia.org/wiki/Jorge_Luis_Borges) [-@borgesjlJardinSenderosQue1941], *The garden of forking paths*. You can find copies of the original short story [here](https://archive.org/stream/TheGardenOfForkingPathsJorgeLuisBorges1941/The-Garden-of-Forking-Paths-Jorge-Luis-Borges-1941_djvu.txt) or [here](https://genius.com/Jorge-luis-borges-the-garden-of-forking-paths-annotated).
...
Throughout this project, we'll make extensive use packages from the [**tidyverse**](https://www.tidyverse.org) for data wrangling and plotting. For tables, we'll use [**gt**](https://gt.rstudio.com/). Much like **ggplot2**, **gt**'s philosophy is use composable parts to create a wide variety tables.
...
If you are new to **tidyverse**-style syntax, possibly the oddest component is the pipe (i.e., `|>`). I'm not going to explain the `|>` in this project, but you might learn more about in the [R4DS chapter on pipes](https://r4ds.hadley.nz/workflow-pipes.html)[@rforda].

Other than the pipe, the other big thing to be aware of is [tibbles](https://tibble.tidyverse.org) [@R-tibble]. For our purposes, think of a tibble as a data object with two dimensions defined by rows and columns. Importantly, tibbles are just special types of [data frames](https://bookdown.org/rdpeng/rprogdatascience/r-nuts-and-bolts.html#data-frames). So, whenever we talk about data frames, we're usually talking about tibbles. For more on the topic, check out [R4DS data transformation chapter](https://r4ds.hadley.nz/data-transform.html).
...
If reshaping data with `pivot_longer()` and `pivot_wider()` is a new concept, check out the vignettes [here](https://tidyr.tidyverse.org/reference/pivot_longer.html) and [here](https://tidyr.tidyverse.org/articles/pivot.html) [@PivotDataWide2020; @Pivoting2020].
...
Note our use of the **gt** package to format the output into a nice table. We'll get more practice with this throughout this chapter.
...
See what I did there with the parentheses? If you assign a value to an object in **R** (e.g., `dog <- 1`) and just hit return, nothing will immediately pop up in the [console](https://r4ds.hadley.nz/intro.html#rstudio). You have to actually execute `dog` before **R** will return `1`. But if you wrap the code within parentheses (e.g., `(dog <- 1)`), **R** will perform the assignment and return the value as if you had executed `dog`.
...
This code got a bit more complex with a switch from **magrittr**'s pipe `%>%` to the base pipe `|>`. The `rowwise()` function is used to apply operations to each row of the data frame. `mutate()` adds three new columns to the data frame: `blue1`, `white`, and `blue2`. These columns are created by applying the `n_blue()` and `n_white()` to all columns of the data frame using `across()`. If you're unfamiliar with `across()`, think of it like `select()` but it works inside of `mutate()` or `summarize()`.
...
t <-
  ...
  rowwise() |>
  mutate(
    blue1 = n_blue(across(everything())),
    white = n_white(across(everything())),
    blue2 = n_blue(across(everything()))
  ) |>
...
(
  d <-
    ...
    mutate(fill = c(
      rep(c("w", "b"), times = c(1, 3)) |>
        rep(times = c(4^0 + 4^1 + 4^2)),
      rep(c("w", "b"), each = 2) |>
        rep(times = c(4^0 + 4^1 + 4^2)),
      rep(c("w", "b"), times = c(3, 1)) |>
        rep(times = c(4^0 + 4^1 + 4^2))
    )) |>
    # now we need to shift the positions over in accordance with draw, as before
    mutate(denominator = ifelse(draw == 1, .5,
                                ifelse(draw == 2, .5 / 4,
                                       .5 / 4^2
                                )
    ...
    # we need to offset to get position axis correct for pie_index == "b" or "c"
...
If you're new to making your own **R** functions, check out [Chapter 19](https://r4ds.had.co.nz/functions.html) of *R4DS* or [Chapter 14](https://bookdown.org/rdpeng/rprogdatascience/functions.html) of *R programming for data science* [@pengProgrammingDataScience2022].
...
(
  lines_1 <-
    ...
    # we need to offset to get position axis correct for pie_index == "b" or "c"
    mutate(
      x = move_over(position = x, index = pie_index),
...
d <-
  d |>
  mutate(remain = c( # pie_index is "a"
    rep(0:1, times = c(1, 3)),
    rep(0, times = 4),
    rep(1:0, times = c(1, 3)) |> rep(times = 3),
    rep(0, times = 4 * 4),
...
<center>plausibility of $p$ after $D_\text{new}$ $\propto$ ways $p$ can produce $D_\text{new} \times$ prior plausibility of $p$.</center>
...
t |>
  ...
  transmute(
    `Possible composition` = str_c("[", d1, " ", d2, " ", d3, " ", d4, "]"),
    p = p,
...
Fair warning: We don't learn the skills for making Figure 2.5 until later in the chapter. So consider the data wrangling steps in this section as something of a preview. This code uses `lag()` from **dplyr**. You can learn more about that [in the **dplyr** documentation](https://dplyr.tidyverse.org/reference/lead-lag.html).
...
To learn more about `dplyr::group_by()` and its opposite `dplyr::ungroup()`, check out [*R4DS* chapter  on data transformations](https://r4ds.hadley.nz/data-transform.html). To learn about `tidyr::expand()`, go [here](https://tidyr.tidyverse.org/reference/expand.html).
...
This stance brushes up against what is sometimes called *mathematical platonism*, which is a position I suspect is casually held among many scientists and laypersons, alike. For more on the topic, check out *Platonism in the philosophy of mathematics* [@linneboPlatonismPhilosophyMathematics2018].
...
For more in this topic, check out McElreath's lecture, [*Understanding Bayesian statistics without frequentist language*](https://youtu.be/yakg94HyWdE).
...
This is the first time we're ursing **brms**. If you haven't already installed **brms**, you can find instructions on how to do so [here](https://github.com/paul-buerkner/brms#how-do-i-install-brms).
...
The **brms** package can handle missing values or measurement error using the `mi()` function. See [this vignette](https://paul-buerkner.github.io/brms/articles/brms_missings.html) on handling missing values for more details. I have not yet been able to get this model to work with **brms** and have [asked the Stan community for help](https://discourse.mc-stan.org/t/misclassification-or-measurement-error-for-binomial-model-with-brms-statistical-rethinking-2023/30012). I will update this page if I figure out how to make it work.
